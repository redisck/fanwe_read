	...
	public function do_refund()
	{
		...
		if($order_item_id)
		{
			...
			if(intval($refund_item_count)==0&&intval($coupon_item_count)==0){
				$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set refund_amount = refund_amount + ".$price.",refund_money = refund_money + ".$price.",refund_score = refund_score + ".$score.",refund_status = 2,after_sale = 1,is_refuse_delivery=0 where id = ".$order_id);
			}else{
				$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set refund_amount = refund_amount + ".$price.",refund_money = refund_money + ".$price.",refund_score = refund_score + ".$score.",is_refuse_delivery=0 where id = ".$order_id);
			}		
		}
		elseif($coupon_id)
		{
			...
			$coupon_item_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_coupon where refund_status = 1 and order_id = ".$order_id);
			if(intval($refund_item_count)==0&&intval($coupon_item_count)==0)
			$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set refund_amount = refund_amount + ".$price.",refund_money = refund_money + ".$price.",refund_score = refund_score + ".$score.",refund_status = 2,after_sale = 1 where id = ".$order_id);
			else
			$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set refund_amount = refund_amount + ".$price.",refund_money = refund_money + ".$price.",refund_score = refund_score + ".$score." where id = ".$order_id);
		...
		{
			require_once APP_ROOT_PATH."system/model/user.php";
			modify_account(array("score"=>$score), $order_info['user_id'],$data['name']."成功退积分");
			order_log($data['name']."积分订单成功退".$score."积分", $order_id);
		}
