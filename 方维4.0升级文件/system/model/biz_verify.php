...
function biz_check_coupon($s_account_info,$pwd,$location_id)
{
		...
		if($coupon_data)
		{
			...
			{
				$result['msg'] = $coupon_data['name']."(购买数量：".$coupon_data['number']."), 消费券有效";
				$result['sub_msg'] = $coupon_data['sub_name']."(购买数量：".$coupon_data['number']."), 消费券有效";
				$result['number'] = $coupon_data['number'];
			}
			else
			{
				$result['msg'] = $coupon_data['name'].", 消费券有效";
				$result['sub_msg'] = $coupon_data['sub_name'].", 消费券有效";
				$result['number'] = 1;
			}
			return $result;
		}
...
function biz_super_use_coupon($s_account_info,$location_id,$pwd,$coupon_use_count){
    ...
    if($use_count_success>1){
        ...
        $data['msg'] = $result['coupon_data']['sub_name']." \n使用成功 ! 消费：".round($coupon_price,2);
    }else
        $data['msg'] = "使用失败";
	return $data;
}
