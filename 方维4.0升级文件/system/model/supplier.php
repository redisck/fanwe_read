...
function get_location_list($limit, $param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0,"supplier_id"=>0), $join = '', $where='',$orderby = '',$field_append="", $groupby='')
{
	...
	if ($groupby) {
	    if($join)
	        $sql = "select ".$tname.".*".$field_append." from ".DB_PREFIX."supplier_location as ".$tname." use index (search_idx1, is_verify) ".$join." where  ".$condition.$groupby;
	    else
	        $sql = "select ".$tname.".*".$field_append." from ".DB_PREFIX."supplier_location as ".$tname." use index (search_idx1, is_verify) where  ".$condition.$groupby;
	}else{
	    if($join)
