...
function load_support_delivery($region_id,$order_id)
{
	...
	if($order_id==0)
	{		
		$is_pick = $GLOBALS['db']->getOne("select min(is_pick) from ".DB_PREFIX."deal_cart where session_id = '".es_session::id()."' and user_id = ".intval($user_info['id']));		
	...
	{
		$is_pick = $GLOBALS['db']->getOne("select min(is_pick) from ".DB_PREFIX."deal_order_item where order_id = '".$order_id."'");		
	...
	if($is_pick==1)
	{
		$pick_delivery = array();
		$pick_delivery['id'] = -1;
		$pick_delivery['name'] = "上门自提";
		$support_delivery_list[] = $pick_delivery;
	}
...
// 		$return_total_score = $return_total_score + $deal_info['return_score'] * $v['number'];
	...
	$deal_item_count = count($goods_list);
	...
	foreach($goods_list_group as $key=>$goods_group)
	{
		...
		foreach($goods_group['goods_list'] as $k=>$v)
		{
			...
			if($deal_info['is_delivery'] == 1&&$is_pick == 0) //需要配送叠加重量
			{
				...
				{
					if($vvv['deal_id']==$v['deal_id'])
					{
						$deal_count+=$vvv['number'];
					}
				}
...
function store_pay_order_paid($order_id)
{
	...
	if($order['pay_amount'] + $order['discount_price'] >=$order['total_price'])
	{
...
function store_pay_order_paid_done($order_id)
{
	...
	$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."store_pay_order where id = ".$order_id);
		...
		$money=$order_info['total_price']-$order_info['discount_price'];
		...
		modify_supplier_account($order_info['total_price']-$order_info['payment_fee'],$order_info['supplier_id'],0,$msg); //商户销售额增加
		modify_supplier_account($money,$order_info['supplier_id'],3,$msg); //商户余额增加
		require_once APP_ROOT_PATH."system/model/store_pay.php";
		store_pay_order_log($order_info['order_sn']."订单付款完成", $order_id);
