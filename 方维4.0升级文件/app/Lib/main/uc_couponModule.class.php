...
class uc_couponModule extends MainBaseModule
{
	public function index()
	{
		...
		if($did>0)
		{
			$order_deal_item = $GLOBALS['db']->getRow("select doi.* from ".$order_item_table." as doi left join ".$order_table." as do on doi.order_id = do.id where doi.id = ".$did." and (doi.is_coupon = 1 or doi.is_pick =1) and do.user_id = ".$user_id);
		...
		$GLOBALS['tmpl']->assign("page_title","我的消费券");
		assign_uc_nav_list();
