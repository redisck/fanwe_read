...
class topicModule extends MainBaseModule
{
	...
	public function reply()
	{
		...
		/*验证每天只允许评论5次*/
 		$day_send_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."topic_reply where and user_id=".$GLOBALS['user_info']['id']." and create_time>".to_timespan(to_date(NOW_TIME,"Y-m-d"),"Y-m-d")." and create_time<".NOW_TIME);
		if($day_send_count>=8){
