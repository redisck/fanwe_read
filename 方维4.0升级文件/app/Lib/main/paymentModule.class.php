...
// 			$order_deals = $GLOBALS['db']->getAll("select d.* from ".DB_PREFIX."deal as d where id in (".$deal_ids.")");
			...
			$order_deals = $GLOBALS['db']->getAll("select d.id,d.uname,d.is_coupon,d.buy_status,d.forbid_sms,doi.name,doi.is_pick from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on d.id = doi.deal_id where doi.order_id = ".$order_id);
