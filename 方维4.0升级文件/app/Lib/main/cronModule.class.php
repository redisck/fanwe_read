...
class cronModule extends MainBaseModule
{
	...
	{		
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."conf set `value` = 1,`sort` = '".NOW_TIME."' where name = 'DEAL_MSG_LOCK' and (`value` = 0 or (`value` = 1 and `sort` <= '".(NOW_TIME-60)."'))");
	...
	public function promote_msg_list()
	{
		...
		//推广队列的群发
		$GLOBALS['db']->query("update ".DB_PREFIX."conf set `value` = 1,`sort` = '".NOW_TIME."' where name = 'PROMOTE_MSG_LOCK' and (`value` = 0 or (`value` = 1 and `sort` <= '".(NOW_TIME-60)."'))");
