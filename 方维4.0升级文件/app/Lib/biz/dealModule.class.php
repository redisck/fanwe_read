...
//             $deal_info = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "deal where id=" . $id); //仅限开放测试使用
            ...
            //关联商品
         	$related_deal_id=$GLOBALS['db']->getOne("select relate_ids from ".DB_PREFIX."relate_goods where good_id = ".$id);
    ...
    public function do_save_publish()
    {
        ...
        $data['description'] = btrim(replace_domain_to_public(no_xss($_REQUEST['description'])));
        $data['notes'] = btrim(replace_domain_to_public(no_xss($_REQUEST['notes'])));
        ...
        $data['coupon_begin_time'] = strim($_REQUEST['coupon_begin_time']) == '' ? 0 : to_timespan($_REQUEST['coupon_begin_time'], "Y-m-d H:i"); // 消费券生效时间:
        $data['coupon_end_time'] = strim($_REQUEST['coupon_end_time']) == '' ? 0 : to_timespan($_REQUEST['coupon_end_time'], "Y-m-d H:i"); // 消费券到期时间
        ...
        $data['coupon_time_type'] = intval($_REQUEST['coupon_time_type']); // 消费券有效期类型
        if ($data['coupon_time_type'] == 1) {
            $data['coupon_begin_time'] = 0;
            $data['coupon_end_time'] = 0;
            $data['coupon_day'] = intval($_REQUEST['coupon_day']); // 消费券有效天数
        } else {
            $data['coupon_day'] = 0;
        }
        
        $data['deal_goods_type'] = intval($_REQUEST['deal_goods_type']); // 消费券商品类型
        ...
        $data['cache_relate']=serialize($relate_array);        
