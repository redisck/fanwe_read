...
// | Fanwe 方维o2o商业系统
// +----------------------------------------------------------------------
// | Copyright (c) 2016 http://www.fanwe.com All rights reserved.
// +----------------------------------------------------------------------
// | Author: hhcycj
// +----------------------------------------------------------------------
...
class stores_payModule extends MainBaseModule
{
        ...
        (
            [0] => Array
                (
                    ...
                    [preview_v1] => 'http://192.168.3.148/fwshop/public/attachment/201304/12/01/5166f2462b537_360x260.jpg' [string] 商户图片 180x130
                    [preview_v2] => 'http://192.168.3.148/fwshop/public/attachment/201304/12/01/5166f2462b537_360x260.jpg' [string] 商户图片 64x64
                    [id] => 22 [int] 商家ID
                    ...
                    [xpoint] => [float] 所在经度
                    [ypoint] => [float] 所在纬度
                    [tel' => '0575-83389799' [string] 电话
                    [dealcate_name] => '休闲娱乐' [string] 分类名称
                    [area_name] => NULL,
                    [promote_description] => '满10000减9999,全程免运费' [string] 优惠描述字符串
                )
        )
	...
	public function index()
	{
		...
		$condition_param = array("open_store_payment"=>$open_store_payment, "cid"=>$catalog_id,"tid"=>$cata_type_id,"aid"=>$area_id,"qid"=>$quan_id,"city_id"=>intval($GLOBALS['city']['id']));
		
		// 设置多表链接
		$join = ' LEFT JOIN '.DB_PREFIX.'promote Promote ON Promote.supplier_id = sl.supplier_id ';
		...
		$field_append .= ", GROUP_CONCAT(Promote.description) as promote_description, Dealcate.name as dealcate_name, slal.area_id as area_id,  area.name as area_name ";
		$group_by = " GROUP BY  sl.id ";
		
		
		require_once APP_ROOT_PATH."system/model/supplier.php";
		$deal_result  = get_location_promote_list($limit,$condition_param, $join,$ext_condition, $group_by, $order,$field_append);
		
		//output($deal_result);
		
		$list = $deal_result['list'];
		$count= $GLOBALS['db']->getOne("select count(distinct(sl.id)) from ".DB_PREFIX."supplier_location as sl ".$join." where ".$deal_result['condition']);
		...
		foreach($list as $k=>$v)
		{
			$fv = format_store_list_item($v);
			$pd_arr = array_unique(explode(',', $v['promote_description']));
			$pd_str = join(',', $pd_arr);
			$fv['promote_description'] = $pd_str;
			$fv['dealcate_name'] = $v['dealcate_name'];
			$fv['area_name'] = $v['area_name'];
			$goodses[$k] = $fv;
		}
