...
class biz_youhuioModule extends MainBaseModule
{
	...
	public function index(){
	    ...
	    $count_sql = 'select count(distinct y.id) as count from '.DB_PREFIX.'youhui y left join '.DB_PREFIX.'youhui_location_link yll on yll.youhui_id = y.id where y.is_effect = 1 AND y.supplier_id =  '.$account_info['supplier_id'].' and yll.location_id in('.implode(",", $account_info['location_ids']).')';
