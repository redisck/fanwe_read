...
class dealModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$order= " d.is_effect desc, d.is_hot desc, d.is_best desc, d.is_new desc ";
		$deal_recommend  = get_deal_list(5, array(DEAL_ONLINE), $condition_param,"", 'd.id <> '.$data_id,$order);
		$temp_deal = '';
		...
		if($supplier_location_list){
		    ...
		    $other_location_deal = get_deal_list(5,array(DEAL_ONLINE,DEAL_NOTICE), '', $join, " dl.location_id in ({$ext_con}) and id <> {$data_id}");
		...
		if($data['allow_promote'] == 1){
		    // 获取1个全站优惠
		    $sql = " select description from ".DB_PREFIX."promote where type = '0'  order by id desc";
		    $promotes = $GLOBALS['db']->getAll($sql);
		    
	        foreach ($promotes as $k=>$v){
	            $promotes_list[] = $v['description'];
	        }
